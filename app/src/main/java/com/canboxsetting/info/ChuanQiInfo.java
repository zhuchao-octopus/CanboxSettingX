...
public class ChuanQiInfo extends MyFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mMainView = inflater.inflate(R.layout.chuanqi_electric_car_infos, container, false);
        initView();
        return mMainView;
    }
    ...
    private void updateView(byte[] buf) {
        ...
        switch (buf[0]) {
            case 0x53: {
                ...
                if (buf[7] == 0 && buf[8] == 0) {
                    s = getString(R.string.distance_charging_time) + "\n" + "--" + getString(R.string.days) + "--" + getString(R.string.gs4_hour) + "--" + getString(R.string.gs4_minute);
                } else {
                    s = getString(R.string.distance_charging_time) + "\n" + ((buf[7] & 0x7) >> 0) + getString(R.string.days) + ((buf[7] & 0xf8) >> 3) + getString(R.string.gs4_hour) + ((buf[8] & 0xfc) >> 2) + getString(R.string.gs4_minute);
                }
                ((TextView) mMainView.findViewById(R.id.distance_charging_time)).setText(s);

                hStart = buf[2] & 0x3f;
                mStart = ((buf[2] & 0xc0) >> 6) | ((buf[3] & 0xff) >> 2);
                hEnd = buf[4] & 0x3f;
                mEnd = ((buf[4] & 0xc0) >> 6) | ((buf[5] & 0xff) >> 2);

                s = String.format("%02d:%02d", hStart, mStart, Locale.ENGLISH);

                ((TextView) mMainView.findViewById(R.id.time_of_appointment1)).setText(s);

                s = String.format("%02d:%02d", hEnd, mEnd, Locale.ENGLISH);

                ((TextView) mMainView.findViewById(R.id.time_of_appointment2)).setText(s);


                s = String.format("%02d:%02d~%02d:%02d", buf[2] & 0x3f, ((buf[2] & 0xc0) >> 6) | ((buf[3] & 0xff) >> 2), buf[4] & 0x3f, ((buf[4] & 0xc0) >> 6) | ((buf[5] & 0xff) >> 2), Locale.ENGLISH);
                ...
                //					.setChecked(false);
                ((CheckBox) mMainView.findViewById(R.id.cycle_date1)).setChecked(false);
                ((CheckBox) mMainView.findViewById(R.id.cycle_date2)).setChecked(false);
                ((CheckBox) mMainView.findViewById(R.id.cycle_date3)).setChecked(false);
                ((CheckBox) mMainView.findViewById(R.id.cycle_date4)).setChecked(false);
                ((CheckBox) mMainView.findViewById(R.id.cycle_date5)).setChecked(false);
                ((CheckBox) mMainView.findViewById(R.id.cycle_date6)).setChecked(false);
                ((CheckBox) mMainView.findViewById(R.id.cycle_date7)).setChecked(false);
                ...
                if ((buf[6] & 0xff) != 0) {
                    ...
                    if ((buf[6] & 0x01) != 0) {
                        ...
                        ((RadioButton) mMainView.findViewById(R.id.cycle_mode1)).setChecked(true);
                    } else {
                        s += getString(R.string.single_time);
                        ((RadioButton) mMainView.findViewById(R.id.cycle_mode2)).setChecked(true);
                    }

                    if ((buf[6] & 0x02) != 0) {
                        s += getString(R.string.monday);

                        ((CheckBox) mMainView.findViewById(R.id.cycle_date1)).setChecked(true);
                    }
                    if ((buf[6] & 0x04) != 0) {
                        s += getString(R.string.tuesday);
                        ((CheckBox) mMainView.findViewById(R.id.cycle_date2)).setChecked(true);
                    }
                    if ((buf[6] & 0x08) != 0) {
                        s += getString(R.string.wednesday);
                        ((CheckBox) mMainView.findViewById(R.id.cycle_date3)).setChecked(true);
                    }
                    if ((buf[6] & 0x10) != 0) {
                        s += getString(R.string.thursday);
                        ((CheckBox) mMainView.findViewById(R.id.cycle_date4)).setChecked(true);
                    }
                    if ((buf[6] & 0x20) != 0) {
                        s += getString(R.string.friday);
                        ((CheckBox) mMainView.findViewById(R.id.cycle_date5)).setChecked(true);
                    }
                    if ((buf[6] & 0x40) != 0) {
                        s += getString(R.string.saturday);
                        ((CheckBox) mMainView.findViewById(R.id.cycle_date6)).setChecked(true);
                    }
                    if ((buf[6] & 0x80) != 0) {
                        s += getString(R.string.sunday);
                        ((CheckBox) mMainView.findViewById(R.id.cycle_date7)).setChecked(true);
                    }
                }

                ((TextView) mMainView.findViewById(R.id.current_appointment)).setText(s);

                s = (buf[10] & 0xff) + " " + getString(R.string.level);
                //
                ((TextView) mMainView.findViewById(R.id.energy_flow_state)).setText(s);

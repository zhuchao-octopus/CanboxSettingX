...
public class ToyotaInfoSimpleFragment extends PreferenceFragment implements
		...
		public void handleMessage(Message msg) {
			if (!mPaused) {
				if ((msg.what & 0xff00) == 0xff00) {
					...
					sendCanboxInfo0x90((msg.what & 0xff00) >> 8,
							msg.what & 0xff);
				}
			}
		}
				...
				public void onProgressChanged(SeekBar seekBar, int progress,
					...
					if (fromUser) {
						if (mTextVolume != null) {
							...
							MachineConfig
							.setIntProperty( SystemConfig.CANBOX_EQ_VOLUME, progress);
						}
					}
				}
			});
	...
	new Node("tpms", 0x0), new Node("hybrid", 0x0),
			new Node("carbodyinfo", 0x0), new Node("car_amplifier_volume", 0x0)
			...
			case 0x1:
				...
				if (string_id != 0) {
					setText(R.id.toyota_car_light,
							getActivity().getString(string_id));
				} else {
					if (!all_visible) {
						mLightStringId = 0;
					}
					if (mLightStringId == 0) {
						...
						setText(R.id.toyota_car_light,
								getActivity().getString(mLightStringId));
					}
				}
...
//			}
		...
		case 0x25:
			...
			if ((buf[2] & 0x3) == 1) {
				...
				tpms = 25 * (buf[3] & 0xff);
				t1 = String.format("%d.%d", tpms / 10,
						(tpms >= 0) ? (tpms % 10) : (-tpms % 10));

				tpms = 25 * (buf[4] & 0xff);
				t2 = String.format("%d.%d", tpms / 10,
						(tpms >= 0) ? (tpms % 10) : (-tpms % 10));
				tpms = 25 * (buf[5] & 0xff);
				t3 = String.format("%d.%d", tpms / 10,
						(tpms >= 0) ? (tpms % 10) : (-tpms % 10));
				tpms = 25 * (buf[6] & 0xff);
				t4 = String.format("%d.%d", tpms / 10,
						(tpms >= 0) ? (tpms % 10) : (-tpms % 10));
				tpms = 25 * (buf[7] & 0xff);
				t5 = String.format("%d.%d", tpms / 10,
						(tpms >= 0) ? (tpms % 10) : (-tpms % 10));

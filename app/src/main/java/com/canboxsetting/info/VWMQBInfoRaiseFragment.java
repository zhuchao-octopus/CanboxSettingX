...
public class VWMQBInfoRaiseFragment extends PreferenceFragment implements
	...
	private static final NodeDrivingData[] NODES_DRIVINGDATA = {
			new NodeDrivingData(R.string.since_start),
			new NodeDrivingData(R.string.long_term),
			new NodeDrivingData(R.string.since_refuelling) };
	...
	private void showStopStartStatusDialog() {
		...
		((TextView) d.findViewById(R.id.pop_text))
				.setText(R.string.start_stop_status);
	...
	private void showVehicletatus() {
		...
		setTextViewStringEx(mMainView.findViewById(R.id.vehicle_status),
				R.id.title, id);
	...
	private void showDrivingData() {
		...
		setTextViewStringEx(mMainView.findViewById(R.id.driving_data),
				R.id.title, n.mId);

		setTextViewStringEx(mMainView.findViewById(R.id.driving_data),
				R.id.avg_speed, n.mAVSpeed);
		setTextViewStringEx(mMainView.findViewById(R.id.driving_data),
				R.id.distance, n.mDistance);
		setTextViewStringEx(mMainView.findViewById(R.id.driving_data),
				R.id.travelling_time, n.mTime);
		setTextViewStringEx(mMainView.findViewById(R.id.driving_data),
				R.id.avg_consumption, n.mOilDistance);
	}
	...
	private static final int[] BUTTON_ON_CLICK = new int[] { R.id.setting,
			R.id.views, R.id.set, };

	private static final int[] POP_LIST = new int[] { R.string.driving_data,
			R.string.conv_consumers, R.string.vehicle_status,
			R.string.energy_flow_view };
	...
	private void initPresentationUI() {
		...
		mMainView.findViewById(R.id.driving_data).findViewById(R.id.bt_left)
				.setOnClickListener(mOnClickDrivingData);
		mMainView.findViewById(R.id.driving_data).findViewById(R.id.bt_right)
				.setOnClickListener(mOnClickDrivingData);
		mMainView.findViewById(R.id.vehicle_status).findViewById(R.id.bt_left)
				.setOnClickListener(mOnClickVehicleStatus);
		mMainView.findViewById(R.id.vehicle_status).findViewById(R.id.bt_right)
				.setOnClickListener(mOnClickVehicleStatus);
		mMainView.findViewById(R.id.vehicle_status)
				.findViewById(R.id.start_stop)
				.setOnClickListener(mOnClickVehicleStatus);
		mMainView.findViewById(R.id.vehicle_status).findViewById(R.id.set)
				.setOnClickListener(mOnClickVehicleStatus);
	...
	private void updateView(byte[] buf) {
		...
		case 0x50:
			...
			case 0x60:
				index = ((buf[4] & 0xff) | ((buf[5] & 0xff) << 8));
				ProgressBar sb = (ProgressBar) mMainView
	...
	private void clearHybird() {
		mBatteryView.findViewById(R.id.canbus8_wheel_motor).setVisibility(
				View.GONE);

		mBatteryView.findViewById(R.id.canbus8_motor_wheel).setVisibility(
				View.GONE);
		mBatteryView.findViewById(R.id.canbus8_battery_motor).setVisibility(
				View.GONE);
		mBatteryView.findViewById(R.id.canbus8_motor_battery).setVisibility(
				View.GONE);
		mBatteryView.findViewById(R.id.canbus8_arrow_set).setVisibility(
				View.GONE);
		mBatteryView.findViewById(R.id.canbus8_img_lf2_1).setVisibility(
				View.GONE);
	}
	...
	private void showHyBird(byte b) {
		...
		if ((mHybrid & 0x20) == 0) {
			...
			if ((mHybrid & 0x2) == 0) {
				mBatteryView.findViewById(R.id.canbus8_motor_wheel)
		...
		if ((mHybrid & 0x10) == 0) {
			mBatteryView.findViewById(R.id.canbus8_battery_motor)
					.setVisibility(View.GONE);

			if ((mHybrid & 0x1) == 0) {
				mBatteryView.findViewById(R.id.canbus8_motor_battery)
						.setVisibility(View.GONE);
			} else {

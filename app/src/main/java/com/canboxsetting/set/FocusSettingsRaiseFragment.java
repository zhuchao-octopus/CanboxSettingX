...
public class FocusSettingsRaiseFragment extends PreferenceFragment implements
		Preference.OnPreferenceChangeListener, OnPreferenceClickListener {
	private static final String TAG = "FocusSettingsFragment";


	
	private static final NodePreference[] NODES_NORMAL = {
			//group1
		new NodePreference("rain_sensor_wiper", 0xC9, 0x28, 0x60, 1),
		new NodePreference("repeat_wiper_once", 0xC9, 0x28, 0x61, 1),
		new NodePreference("rear_wiper", 0xC9, 0x28, 0x62, 1),
		...
		new NodePreference("blind_spot_monitoring", 0xC9, 0x28, 0x13, 2),
		new NodePreference("fatigue_driving_warning", 0xC9, 0x28, 0x14, 2),
		...
		new NodePreference("incoming_reverse_gear_warning", 0xC9, 0x28, 0x2, 3),
		new NodePreference("tcs_traction_control", 0xC9, 0x28, 0x3, 3),
		new NodePreference("automatic_engine_shutdown", 0xC9, 0x28, 0x5, 3),
		
		//4

		new NodePreference("switch_prohibited", 0xC9, 0x28, 0x50, 4),
		new NodePreference("voice_feedback", 0xC9, 0x28, 0x51, 4),
		new NodePreference("false_lock_warning", 0xC9, 0x28, 0x52, 4),
		new NodePreference("remote_unlock", 0xC9, 0x28, 0x53, 4, R.array.door_to_be_unlocked_entries, R.array.brightness_control_value),

		new NodePreference("str_auto_unlock", 0xC9, 0x28, 0x54, 4),
		new NodePreference("remote_control_on", 0xC9, 0x28, 0x55, 4),
		new NodePreference("remote_control_off", 0xC9, 0x28, 0x56, 4),
		new NodePreference("activate_remote_start", 0xC9, 0x28, 0x57, 4),
		new NodePreference("air_control", 0xC9, 0x28, 0x58, 4, R.array.air_control_entries, R.array.brightness_control_value),
		...
		new NodePreference("automatic_high_beam", 0xC9, 0x28, 0x22, 5),
		new NodePreference("electric_trunk", 0xC9, 0x28, 0x30, 5),
		new NodePreference("auto_fold_wing_mirror", 0xC9, 0x28, 0x40, 5),
		
//public NodePreference(String id, int cmd, int status, int mask, int type, int seekMin, int seekMax, int step) {
		
		new NodePreference("ambient_light_color", 0xC5, 0x61, 0x7f, 5, R.array.atmosphere_raise_light_color_entries, R.array.eninstrument_style3_value),
		new NodePreference("ambient_light_brightness", 0xC5, 0x61, 0x7f, 5, 0, 100, 1),
		
		//6

		new NodePreference("korea_amp_treble", 0xC3, 0x0, 0x0, 0x06, -7, 7, 1),
		new NodePreference("korea_amp_middle", 0xC3, 0x1, 0x0, 0x06, -7, 7, 1),
		new NodePreference("korea_amp_bass", 0xC3, 0x2, 0x0, 0x06, -7, 7, 1),
		new NodePreference("attenuation", 0xC3, 0x3, 0x0, 0x06, -7, 7, 1),
...
//		new NodePreference("korea_amp_volume", 0xC3, 0x4, 0x0, 0x106, 0, 30, 1),
	...
	private static final NodePreference[] NODES_LINGJIE = {
			//group1
		new NodePreference("rain_sensor_wiper", 0xC9, 0x28, 0x60, 1),
		new NodePreference("repeat_wiper_once", 0xC9, 0x28, 0x61, 1),
		new NodePreference("rear_wiper", 0xC9, 0x28, 0x62, 1),
...
//public NodePreference(String id, int cmd, int status, int mask, int type, int seekMin, int seekMax, int step) {
		...
		new NodePreference("ambient_light_color", 0xC5, 0x61, 0x7f, 5, R.array.atmosphere_bell_light_color_entries, R.array.eight_values),
		new NodePreference("ambient_light_brightness", 0xC5, 0x61, 0x7f, 5, 0, 100, 1),
...
//		new NodePreference("korea_amp_volume", 0xC3, 0x4, 0x0, 0x106, 0, 30, 1),
				...
				if (index < getPreferenceScreen().getPreferenceCount()) {
					...
					if (ps instanceof PreferenceScreen) {
						...
						if ((p instanceof ListPreference)
								...
								|| (p instanceof MyPreferenceSeekBar)) {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(R.xml.focus_raise_settings);

		init();
		
		findPreference("information_tone_volume")
...
//		sendCanboxInfo(0xa2, mLedSeting);
	...
	private void udpateShowWarning() {
		int show = Settings.System.getInt(getActivity().getContentResolver(),
				SystemConfig.SHOW_FOCUS_CAR_WARNING_MSG, 0);

		if (show != 0) {
			((SwitchPreference) findPreference("off_Warning_info"))
					.setChecked(true);
		}
	...
	public boolean onPreferenceChange(Preference preference, Object newValue) {
		...
		try {
			...
			} else if ("ambient_light_color".equals(key)) {
				sendCanboxInfo(0xc5, Integer.parseInt((String) newValue),
						m61[1]);
			} else  {
			...
			if (NODES[i].mKey.equals(key)) {
				if (preference instanceof ListPreference) {	
					...
					try{
						...
						if ("ambient_light_brightness".equals(key)) {
							sendCanboxInfo(0xc5, m61[0],
									v);
						} else {
							sendCanboxInfo(NODES[i].mCmd, NODES[i].mStatus,
								v);
						}
	...
	private boolean isEntryValue(CharSequence []ss, int value){
		boolean ret = false;
		for (CharSequence s : ss){
			try{
				int v = Integer.valueOf(s.toString());
				if (v == value){
					ret = true;
					break;
				}
	...
	private void setPreference(String key, int index) {
		...
		if (p != null) {
			if (p instanceof ListPreference) {
				...
				CharSequence []ss = lp.getEntryValues();
//				if (ss != null && (ss.length > index)) {
				if (isEntryValue(ss, index)){
	...
	private void updateView(byte[] buf) {
		...
		if (buf[0] == 0x62){
			...
			setPreference("ambient_light_color", buf[2] & 0xf);
			setPreference("ambient_light_brightness", buf[3]);

			m61[0] = (byte)(buf[2] & 0xf);
			m61[1] = buf[3];
			return;
		}
		...
		case 0x24:
			if ((buf[4] & 0x4) != 0) {
				((SwitchPreference) findPreference("information_tone_volume"))
						.setChecked(true);
			} else {
				((SwitchPreference) findPreference("information_tone_volume"))
						.setChecked(false);
			...
			if ((buf[4] & 0x1) != 0) {
				((SwitchPreference) findPreference("control_system"))
						.setChecked(true);
			} else {
				((SwitchPreference) findPreference("control_system"))
						.setChecked(false);
			...
			if ((buf[4] & 0x8) != 0) {
				((SwitchPreference) findPreference("warning_tone"))
						.setChecked(true);
			} else {
				((SwitchPreference) findPreference("warning_tone"))
						.setChecked(false);
			...
			((ListPreference) findPreference("turn_lights_set"))
					.setValue(String.valueOf(index));
			((ListPreference) findPreference("turn_lights_set"))
					.setSummary(((ListPreference) findPreference("turn_lights_set"))
							.getEntry());

			index = (int) ((buf[4] & 0x80) >> 7);
			((ListPreference) findPreference("mileage_unit")).setValue(String
					.valueOf(index));
			((ListPreference) findPreference("mileage_unit"))
					.setSummary(((ListPreference) findPreference("mileage_unit"))
							.getEntry());
		...
		case 0x21:
			...
			((ListPreference) findPreference("temp_dis_k")).setValue(String
					.valueOf(index));

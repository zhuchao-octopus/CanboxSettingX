...
public class Golf7SettingsSimpleFragment extends PreferenceFragmentCompat implements Preference.OnPreferenceChangeListener, OnPreferenceClickListener {
    ...
    private void updateView(byte[] buf) {
        ...
        if ((buf[0] & 0xff) == 0x40 && (buf[2] & 0xff) == 0xa0) {// driver
            ...
            if ((buf[3] & 0x3) == 3) {
                ((SwitchPreference) findPreference("individual")).setChecked(true);
                ...
                if ((buf[3] & 0x3) == 0) {
                    ((SwitchPreference) findPreference("normal")).setChecked(true);
                } else if ((buf[3] & 0x3) == 1) {
                    ((SwitchPreference) findPreference("sport")).setChecked(true);
                }
                if ((buf[3] & 0x3) == 2) {
                    ((SwitchPreference) findPreference("driv_eco")).setChecked(true);
                }

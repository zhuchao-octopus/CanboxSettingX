...
public class HondaSettingsRaiseFragment extends PreferenceFragment implements
	...
	private static final Node[] NODES = {
			...
			new Node("adjust_outside", 0xC600, 0x32000000, 0x000f, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("trip_a", 0xC602, 0x32000000, 0x0030, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("trip_b", 0xC603, 0x32000000, 0x00c0, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("interior", 0xC604, 0x32000000, 0x0103, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("headlight", 0xC605, 0x32000000, 0x010c, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("auto_light", 0xC606, 0x32000000, 0x0170, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("lock_with", 0xC607, 0x32000000, 0x0203, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("unlock_with", 0xC608, 0x32000000, 0x020c, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("key_mode", 0xC609, 0x32000000, 0x0240, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("keyless", 0xC60a, 0x32000000, 0x0280, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("security", 0xC60b, 0x32000000, 0x0230, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("access_beep", 0xC60d, 0x32000000, 0x0340, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("tpms_check", 0xC61100),
			new Node("default_all", 0xC60f00),

			new Node("alarm_vol", 0xC612, 0x32000000, 0x04c0, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("backlight", 0xC613, 0x32000000, 0x0420, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("notifications", 0xC614, 0x32000000, 0x0410, 0x0410,
					Node.TYPE_BUFF1_INDEX),
			new Node("speed_distance_units", 0xC615, 0x32000000, 0x0408, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("tachometer", 0xC616, 0x32000000, 0x0404, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("walk_away_auto_lock", 0xC617, 0x32000000, 0x0402, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("remote_startoff", 0xC618, 0x32000000, 0x0320, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("door_unlock_mode", 0xC619, 0x32000000, 0x0310, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("keyless_access_light", 0xC61a, 0x32000000, 0x0308, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("illumination", 0xC61b, 0x32000000, 0x0307, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("headlight_wiper", 0xC61c, 0x32000000, 0x0401, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("start_stop_dis", 0xC61d, 0x32000000, 0x0540, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("voice_alarm", 0xC61e, 0x32000000, 0x0580, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("danger_ahead", 0xC61f, 0x32000000, 0x050c, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("acc_car", 0xC620, 0x32000000, 0x0520, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("stop_lkas", 0xC621, 0x32000000, 0x0510, 0x0,
					Node.TYPE_BUFF1_INDEX),
			new Node("deviate", 0xC622, 0x32000000, 0x0503, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("tachometer_settings", 0xC623, 0x32000000, 0x0680, 0x0,
					Node.TYPE_BUFF1_INDEX),

			new Node("driver_attention_monitor", 0xC624, 0x32000000, 0x0660,
					0x0, Node.TYPE_BUFF1_INDEX),
			new Node("the_handle_electrically", 0xC626, 0x32000000, 0x0680,
					0x0, Node.TYPE_BUFF1_INDEX),
			new Node("remote_control_open_condition", 0xC625, 0x32000000,
					0x0610, 0x0, Node.TYPE_BUFF1_INDEX),

			new Node("reset_info", 0xC60e00),
			

			new Node("trunk_lid_induction_opening_closing_function", 0xC646, 0x32000000,
					0x0a80, 0x0, Node.TYPE_BUFF1_INDEX),
			new Node("view_mirror_automatically_folded", 0xC647, 0x32000000,
					0x0a40, 0x0, Node.TYPE_BUFF1_INDEX),
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(R.xml.honda_raise_setting);
	...
	private void updateView(byte[] buf) {
		...
		if ((buf[0] & 0xff) == 0xd0) {
			if (buf[2] == 0x60/* && buf[3] == 0 */) {// bad...
				if (mSetCTM == 1) {
					((SwitchPreference) findPreference("ctm_system"))
							.setChecked(true);
				} else {
					((SwitchPreference) findPreference("ctm_system"))
							.setChecked(false);

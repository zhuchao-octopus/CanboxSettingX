...
public class MazdaRaiseSettingsFragment extends PreferenceFragment implements
	...
	private static final Node[] NODES = {
			...
			new Node("mazda3_2020_settings_c", 0xA40b01, 0x7408, 0x80, 0x1),
			new Node("mazda3_2020_settings_d", 0xA40b02, 0x7408, 0x60, 0x1),
			new Node("mazda3_2020_settings_e", 0xA40b03, 0x7408, 0x18, 0x1),
			new Node("safe_reset", 0xA40b04, 0, 0, 0),

			new Node("mazda3_2020_settings_10", 0xA40701, 0x7405, 0x80, 0x1),
			new Node("mazda3_2020_settings_11", 0xA40702, 0x7405, 0x60, 0x1),
			new Node("mazda3_2020_settings_12", 0xA40703, 0x7405, 0x18, 0x1),
			new Node("speed_warning_reset", 0xA40704, 0, 0, 0),

			new Node("homeinmode", 0x8381, 0x4107, 0x70, 0x0),
			new Node("homeoutmode", 0x8380, 0x4107, 0x80, 0x0),
			new Node("backwindowdefog", 0x8316, 0x4105, 0x08, 0x0),
			new Node("mazda3_2020_settings_1", 0x8317, 0x4105, 0x07, 0x0),
			new Node("mazda3_2020_settings_2", 0xA40801, 0x7407, 0x80, 0x1),
			new Node("mazda3_2020_settings_3", 0xA40902, 0x7407, 0x38, 0x1),
			new Node("psa_simple_17", 0x831a, 0x4106, 0x0c, 0x0),
			new Node("mazda3_2020_settings_4", 0x8319, 0x4106, 0x30, 0x0),
			new Node("mazda3_2020_settings_5", 0xA40901, 0x7407, 0x40, 0x1),
			new Node("mazda3_2020_settings_6", 0xA40a, 0x7407, 0x04, 0x1),
			new Node("mazda3_2020_settings_8", 0xA40c01, 0x7409, 0x80, 0x1),
			new Node("mazda3_2020_settings_9", 0xA40c02, 0x7409, 0x40, 0x1),
			new Node("mazda3_2020_settings_a", 0xA40c03, 0x7409, 0x20, 0x1),

			new Node("rear_view", 0x8318, 0x4106, 0x40, 0x0),
			new Node("daylight", 0x8385, 0x4107, 0x01, 0x0),
			new Node("range", 0x8382, 0x4107, 0x08, 0x0),
			new Node("temp", 0x8383, 0x4107, 0x04, 0x0),
			new Node("background_lighting", 0x8386, 0x4108, 0xc0, 0x0),
			new Node("mazda3_2020_settings_7", 0x8313, 0x4105, 0x30, 0x0),
			...
			//
			new Node("auto_light_open", 0x830f, 0x4104, 0x1c, 0x0),
			new Node("car_light_off", 0x830c, 0x4103, 0x0c, 0x0),
			//
			new Node("adaptive", 0x830e, 0x4103, 0x02, 0x0),
			new Node("turn_volume", 0x8308, 0x4102, 0x60, 0x0),
			new Node("three_signal", 0x8307, 0x4102, 0x80, 0x0),
			//
			new Node("automatic_latch", 0x8305, 0x4101, 0x80, 0x0),
			new Node("lock_mode", 0x8303, 0x4100, 0x04, 0x0),
			//
			...
			//
			new Node("reset_avg", 0x8315, 0, 0, 0x0),
			new Node("sync_avg", 0x8314, 0x4106, 0x80, 0x0),
			//
			...
			new Node("brightness_control", 0x8404, 0x4001, 0xff, 0x0),
			new Node("height2", 0x8403, 0x4002, 0xff, 0x0),

			new Node("mazda3_2020_settings_13", 0x8405, 0x4003, 0x7f, 0x0),
			new Node("mazda3_2020_settings_14", 0x8408, 0x4004, 0x78, 0x0),
			new Node("display_reset", 0x8407, 0x0, 0x0, 0x0),
			//
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		int i = SystemConfig.getIntProperty(getActivity(),
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			if (!mPaused) {
				...
				byte[] buf = new byte[] { (byte) ((msg.what & 0xff00) >> 8),
						0x01, (byte) (msg.what & 0xff) };
			...
			if (NODES[i].mKey.equals(key)) {
				if (preference instanceof ListPreference) {
					...
					if (NODES[i].mType == Node.TYPE_CUSTOM) {
						...
						SystemConfig.setIntProperty(getActivity(),
								SystemConfig.KEY_CANBOX_TOUCH_PANNEL, ((Boolean) newValue) ? 0x0 : 0x1);
						((SwitchPreference) preference).setChecked(((Boolean) newValue));
					} else {
						if ("mazda3_2020_settings_2".equals(key)
								|| "mazda3_2020_settings_5".equals(key)
								|| "mazda3_2020_settings_8".equals(key)
								|| "mazda3_2020_settings_9".equals(key)
								|| "mazda3_2020_settings_a".equals(key)
								|| "mazda3_2020_settings_c".equals(key)
								|| "mazda3_2020_settings_10".equals(key)) {
							sendCanboxData(NODES[i].mCmd,
	...
	private void sendCanboxInfo(int d0, int d1, int d2, int d3) {
		byte[] buf = new byte[] { (byte) d0, 0x03, (byte) d1, (byte) d2,
				(byte) d3 };
		BroadcastUtil.sendCanboxInfo(getActivity(), buf);
	}

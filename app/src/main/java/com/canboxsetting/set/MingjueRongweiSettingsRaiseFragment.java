...
public class MingjueRongweiSettingsRaiseFragment extends PreferenceFragment implements
	...
	private static final Node[] NODES = {
		...
		new Node("key_driving_lock", 0xc60101, 0x39, 0x0, 0x0),
		new Node("key_unlock", 0xc60102, 0x39, 0x0, 0x0),
		new Node("key_unlock_mode", 0xc60103, 0x39, 0x0, 0x0),
		new Node("key_unlock_near_car", 0xc60104, 0x39, 0x0, 0x0),
		new Node("key_flow_me_home", 0xc60201, 0x39, 0x0, 0x0),
		new Node("key_flow_me_home_times", 0xc60202, 0x4001, 0xf, 0x0),
		new Node("key_search_car_indicator", 0xc60203, 0x39, 0x0, 0x0),
		new Node("key_search_car_indicator_times", 0xc60204, 0x4002, 0xf, 0x0),
		
		
		
		new Node("key_flow_me_home_switch", 0xc60301, 0x4100, 0x80, 0x0),
		new Node("key_search_car_indicator_type", 0xc60302, 0x4100, 0x40, 0x0),
		new Node("key_steering_handle", 0xc60303, 0x4100, 0x30, 0x0),
		new Node("greeting_lights", 0xc60304, 0x4108, 0x80, 0x0),
		
		new Node("reduction_of_fetal_pressure_key", 0xc60401, 0x4102, 0x80, 0x0),
		new Node("body_stability_control", 0xc60402, 0x4102, 0x40, 0x0),
		
		new Node("rear_window_defrost_defogging_linkage_key", 0xc60501, 0x39, 0x0, 0x0),
		new Node("volumeset_key", 0xc60502, 0x39, 0x0, 0x0),
		new Node("air_temperature_settings_key", 0xc60503, 0x4101, 0x18, 0x0),
		
			new Node("automatic_folding_of_rearview_mirror_key", 0xc60801,
					0x39, 0x0, 0x0),
			...
			new Node("warning_pedestrian_cues", 0xc60601, 0x39, 0x0, 0x0),
			new Node("warning_pedestrian_cues", 0xc60601, 0x39, 0x0, 0x0),
			

			new Node("sunroof", 0xc60901, 0x39, 0x0, 0x0),
			new Node("front_left_window", 0xc60902, 0x39, 0x0, 0x0),
			new Node("rear_left_window", 0xc60903, 0x39, 0x0, 0x0),
			new Node("front_right_window", 0xc60904, 0x39, 0x0, 0x0),
			new Node("rear_right_window", 0xc60905, 0x39, 0x0, 0x0),
			
			new Node("opening_degree_of_reserve_box", 0xc60906, 0x39, 0x0, 0x0),
			new Node("language1", 0xc60a01, 0x39, 0x0, 0x0),
			
			new Node("driving_mode", 0xc60b01, 0x5300, 0x0, 0x0),


			new Node("dynamic_response", 0xc60b04, 0x5301, 0x0, 0x0),
			new Node("steering_feel", 0xc60b04, 0x5301, 0x0, 0x0),
			new Node("comfortable_air_conditioning", 0xc60b04, 0x5301, 0x0, 0x0),
			...
			if (NODES[i].mKey.equals(key)) {
				if (preference instanceof ListPreference) {
					...
					if ("driving_mode".equals(NODES[i].mKey)){
						...
						sendDriverMode();((ListPreference)preference).setValue(String.valueOf(v));
						((ListPreference)preference).setSummary("%s");
		...
		if (buf[0] == 0x53) {
			...
			for (int i = 0; i < NODES.length; ++i) {
				...
				if (NODES[i].mStatus <= 0xff) {
					...
					if (cmd == (buf[0] & 0xff) && param1 == (buf[2] & 0xff)
							&& param2 == (buf[3] & 0xff)) {
						value = buf[4];

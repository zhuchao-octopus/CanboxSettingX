        ...
        for (int i = 0; i < NODES.length; ++i) {
            if (NODES[i].mKey.equals(key)) {
                if (preference instanceof ListPreference) {
                    ...
                    sendCanboxData(NODES[i].mCmd, NODES[i].mMask, Integer.parseInt((String) newValue));

                } else if (preference instanceof SwitchPreference) {

                    sendCanboxData(NODES[i].mCmd, NODES[i].mMask, ((Boolean) newValue) ? 0x1 : 0x2);
    ...
    private void updateView(byte[] buf) {
        ...
        for (int i = 0; i < NODES.length; ++i) {
            ...
            if (NODES[i].mType == Node.TYPE_DEF_BUFF) {
                if ((buf[0] & 0xff) == NODES[i].mStatus && (buf[2] & 0xff) == NODES[i].mMask) {
                    value = (buf[3] & 0xff);

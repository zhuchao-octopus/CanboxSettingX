...
public class PsaSettingHiworldFragment extends PreferenceFragmentCompat implements Preference.OnPreferenceChangeListener, OnPreferenceClickListener {
    ...
    private static final Node[] NODES = {
            // 灯光设置
            new Node("str_adaptive_front_lighting", 0x7D02, 0x79000000, 0x40, 0, Node.TYPE_BUFF1),

            new Node("str_daytime_running_lamp", 0x7B05, 0x76000001, 0x04, 0, Node.TYPE_BUFF1),

            new Node("str_atmosphere_lighting", 0x7B0A, 0x76000000, 0x8, 0, Node.TYPE_BUFF1),

            new Node("theme_color", 0x7D09, 0x79000000, 0x1, 0, Node.TYPE_BUFF1),

            new Node("str_go_home", 0x7B06, 0x76000001, 0x3, 0, Node.TYPE_BUFF1),

            new Node("str_welcome_lighting", 0x7B09, 0x76000000, 0x30, 0, Node.TYPE_BUFF1),

            new Node("atmosphere_model", 0x7D0A, 0x79000001, 0xc0, 0, Node.TYPE_BUFF1),

            // 车锁设置
            new Node("str_auto_lock", 0x7B0C, 0x76000001, 0x20, 0, Node.TYPE_BUFF1),

            new Node("str_the_door_locked", 0x7B0D, 0x76000001, 0x10, 0, Node.TYPE_BUFF1),

            new Node("only_set_the_trunk", 0x7D01, 0x79000000, 0x80, 0, Node.TYPE_BUFF1),

            new Node("str_remote_unlock_style", 0x7B04, 0x76000001, 0x8, 0, Node.TYPE_BUFF1),

            // 车声设置
            new Node("psa_parking_assistance", 0x7B02, 0x76000001, 0x40, 0, Node.TYPE_BUFF1),

            new Node("str_rear_wiper", 0x7B01, 0x76000001, 0x80, 0, Node.TYPE_BUFF1),

            new Node("str_radar_inactive", 0x7B0B, 0x76000000, 0x40, 0, Node.TYPE_BUFF1),

            new Node("oil_unit", 0xCA05, 0xc1000001, 0x6, 0, Node.TYPE_BUFF1),

            new Node("str_auto_start_stop", 0x8C01, 0x85000001, 0x80, 0, Node.TYPE_BUFF1),

            new Node("welcome_cmd", 0x7D05, 0x79000000, 0x10, 0, Node.TYPE_BUFF1),

            new Node("tpms_calibration", 0x7D03, 0x62010000, 0x300000, 0, Node.TYPE_BUFF1),

            new Node("str_auto_hold", 0x7B08, 0x76000000, 0x80, 0, Node.TYPE_BUFF1),

            new Node("str_lane_changing_assistant", 0x7D04, 0x79000000, 0x20, 0, Node.TYPE_BUFF1),

            new Node("str_temperature_unit", 0xCA03, 0xc1000001, 0x20, 0, Node.TYPE_BUFF1),

            new Node("fatigue_detection_system", 0x7D07, 0x79000000, 0x04, 0, Node.TYPE_BUFF1),

            new Node("lane_keeping_auxiliary", 0x7D06, 0x79000000, 0x8, 0, Node.TYPE_BUFF1),

            new Node("speed_limit_prompt", 0x7D08, 0x79000000, 0x2, 0, Node.TYPE_BUFF1),

            new Node("psa_driving_mode", 0x7D0B, 0x79000001, 0x20, 0, Node.TYPE_BUFF1),

            new Node("ion_purifier", 0x7D0C, 0x79000001, 0x18, 0, Node.TYPE_BUFF1),

            new Node("smoked_type", 0x7D0D, 0x79000001, 0x6, 0, Node.TYPE_BUFF1),

            new Node("fumigating_concentration", 0x7D0E, 0x79000002, 0xC0, 0, Node.TYPE_BUFF1),

            new Node("nitialization_mode", 0x7D0F, 0x9C010000, 0xFF00, 0, Node.TYPE_BUFF1),
            // 功放设置
            new Node("sound_distribution", 0xAD0B, 0xffffffff, 0, 0, Node.TYPE_BUFF1),

            new Node("loudness", 0xAD0C, 0xffffffff, 0, 0, Node.TYPE_BUFF1),
            ...
            if (NODES[i].mKey.equals(key)) {
                if (preference instanceof ListPreference) {
                    if ("oil_unit".equals(key)) {
                        ...
                        sendCanboxData(NODES[i].mCmd, Integer.parseInt((String) newValue) + 1);
                    } else {
                        sendCanboxData(NODES[i].mCmd, Integer.parseInt((String) newValue));
                    }

                    if (NODES[i].mStatus == 0xffffffff) {
                        setPreference(NODES[i].mKey, Integer.parseInt((String) newValue));
                    }
                } else if (preference instanceof SwitchPreference) {
                    if ("str_atmosphere_lighting".equals(key)) {
                        sendCanboxData(NODES[i].mCmd, ((Boolean) newValue) ? 0x80 : 0x0);
                    } else {

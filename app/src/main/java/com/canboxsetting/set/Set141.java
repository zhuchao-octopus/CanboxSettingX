...
public class Set141 extends PreferenceFragment implements
	...
	private static final NodePreference[] NODES = {
			...
			new NodePreference("daytime_driving_lights", 0x860101, 0x37, 0xff,
					0),

			new NodePreference("myhome", 0x860102, 0x37, 0xff, 0,
					R.array.baic_go_home, R.array.six_values),

			new NodePreference("lane_change_flashes", 0x860103, 0x37, 0xff, 0,
					R.array.light_one_key_turn_entries2, R.array.three_values),
					
//
			new NodePreference("zhonghua_xx", 0x860201, 0x37,
					0xff, 0, R.array.baic_automatic_locking,
					R.array.five_values),

			new NodePreference("prompt_for_successful_lock", 0x860202, 0x37,
					0xff, 0),
			new NodePreference("prompt_for_lock_failure", 0x860203, 0x37, 0xff,
					0),
			new NodePreference("power_off_to_unlock", 0x860204, 0x37, 0xff, 0),

			new NodePreference("low_power_settings", 0x860301, 0x37, 0xff, 0),
			new NodePreference("energy_recovery_intensity", 0x860302, 0x37,
					0xff, 0, R.array.energy_recovery_intensity_entries,
					R.array.three_values),
					
			new NodePreference("blind_spot_detector", 0x860401, 0x37, 0xff, 0),
			new NodePreference("chana_lane_departure_warning", 0x860402, 0x37, 0xff, 0),	
			
			new NodePreference("alarm_mode", 0x860403, 0x37,
					0xff, 0, R.array.baic_interface_alarm_and_sound_alarm,
					R.array.two_values),

			new NodePreference("lane_departure_sensitivity", 0x860404, 0x37,
					0xff, 0, R.array.steering_signal_volume_entries,
					R.array.two_values),
					
			new NodePreference("signal_pedal", 0x860501, 0x37, 0xff, 0),
			new NodePreference("creep", 0x860502, 0x37, 0xff, 0),
		...
		for (int i = 0; i < NODES.length; ++i) {
			if (NODES[i].mKey.equals(key)) {
				if (preference instanceof ListPreference) {
						...
						sendCanboxInfo((NODES[i].mCmd&0xff0000) >> 16, (NODES[i].mCmd&0xff00) >> 8, NODES[i].mCmd&0xff,
...
//		sendDataToCanbox(buf, buf.length);
	...
	private void sendCanboxInfo(int cmd, int d0, int d1, int d2) {
		...
		byte[] buf = new byte[] { (byte) cmd, 0x3, (byte) d0, (byte) d1,
				(byte) d2 };
		BroadcastUtil.sendCanboxInfo(getActivity(), buf);

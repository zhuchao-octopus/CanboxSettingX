...
public class Set7 extends PreferenceFragment implements
		...
		for (int i = 0; i < NODES.length; ++i) {
			if (NODES[i].mKey.equals(key)) {
				if (preference instanceof ListPreference) {
					...
					byte[] buf = new byte[] {
							(byte) ((NODES[i].mCmd & 0xff00) >> 8), 0x2,
							(byte) (NODES[i].mCmd & 0xff), (byte) value };
					...
					byte[] buf = new byte[] {
							(byte) ((NODES[i].mCmd & 0xff00) >> 8), 0x2,
							(byte) (NODES[i].mCmd & 0xff),
							(byte) (((Boolean) newValue) ? 0x1 : 0x0) };
					BroadcastUtil.sendCanboxInfo(getActivity(), buf);

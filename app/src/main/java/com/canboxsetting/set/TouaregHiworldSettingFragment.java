...
public class TouaregHiworldSettingFragment extends PreferenceFragment implements
	...
	private static final Node[] NODES = {
		...
		new Node("wiper", 0xcc02, 0xc3, 10, Node.TYPE_BUFF1),		
		new Node("reset", 0xcc01, 0, 0, 0),
		
	new Node("fvol", 0x7a01, 0x71, 0x3, Node.TYPE_BUFF1),
			new Node("fvol_on", 0x7a02, 0x71, 0x4, Node.TYPE_BUFF1),
			new Node("bvol", 0x7a03, 0x71, 0x5, Node.TYPE_BUFF1),
			new Node("bvol_on", 0x7a04, 0x71, 0x6, Node.TYPE_BUFF1),

			new Node("fwindow", 0x6f01, 0x64, 0x800000, 0x0),
			new Node("bwindow", 0x6f02, 0x64, 0x400000, 0x0),
			new Node("hwindow", 0x6f03, 0x64, 0x200000, 0x0),

			new Node("autolockcmd", 0x6f05, 0x64, 0x80000000, 0x0),

			new Node("rearview_mirror", 0x6f06, 0x64, 0x40000000, 0x0),
			new Node("trunklock", 0x6f08, 0x64, 0x10000000, 0x0),
			new Node("doorlock", 0x6f07, 0x64, 0x20000000, 0x0),

			new Node("auto_active", 0x7a05, 0x71, 0x8000, 0x0),

			new Node("speed_warning", 0x4b02, 0x46, 0x8000, 0x0),
			new Node("warning_at", 0x4b03, 0x46, 0xff0000, 0x0),

			new Node("raining", 0x4d01, 0x48, 0x8000, 0x0),

			new Node("incar_light", 0x6d11, 0x68, 0x6, Node.TYPE_BUFF1),

			new Node("gohome", 0x6d0d, 0x68, 0xc000000, 0),
			new Node("leavehome", 0x6d0e, 0x68, 0x3000000, 0),

			new Node("foot", 0x6d0e, 0x68, 0x5, Node.TYPE_BUFF1),

			new Node("car_light", 0x6d10, 0x68, 0x10000000, 0),
			new Node("intelligent_driving", 0x6d12, 0x68, 0x20000000, 0),
			new Node("left_drive", 0x6d13, 0x68, 0x40000000, 0),

			new Node("range", 0xca01, 0xc1, 0x8000, Node.TYPE_BUFF2),
			new Node("speed", 0xca02, 0xc1, 0x4000, Node.TYPE_BUFF2),
			new Node("temp", 0xca03, 0xc1, 0x2000, Node.TYPE_BUFF2),
			new Node("volume", 0xca04, 0xc1, 0x1800, Node.TYPE_BUFF2),
			new Node("energy", 0xca05, 0xc1, 0x0600, Node.TYPE_BUFF2),
			new Node("tpms", 0xca06, 0xc1, 0xc00000, Node.TYPE_BUFF2),

			new Node("air_switch", 0x3a06, 0x31, 0x40, 0),

			new Node("auto_air", 0x3a01, 0x31, 0x800, 0),

			new Node("blow_window", 0x3a03, 0x31, 5, Node.TYPE_BUFF3),
			new Node("blowing", 0x3a04, 0x31, 5, Node.TYPE_BUFF3),
			new Node("feet", 0x3a05, 0x31, 5, Node.TYPE_BUFF3),
			...
			if (NODES[i].mStatus == (buf[2] & 0xff)) {
				if (NODES[i].mShow == Node.TYPE_BUFF1) {
					...
					if (NODES[i].mKey.equals("blow_window")) {
						if (value == 0xb || value == 0xc || value == 0xd
								|| value == 0xe) {
							value = 1;
						} else {
							value = 0;
						...
						if (value == 0x5 || value == 0x6 || value == 0xd
								|| value == 0xe) {
							value = 1;
						} else {
							value = 0;
						...
						if (value == 0x3 || value == 0x5 || value == 0xc
								|| value == 0xe) {
							value = 1;
						} else {
							value = 0;

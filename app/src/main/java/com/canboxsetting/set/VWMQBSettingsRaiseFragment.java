...
public class VWMQBSettingsRaiseFragment extends PreferenceFragment implements
	...
	private static final Node[] NODES = {
			...
			new Node("speed_adjustment", 0xa806, 0x51000000, 0x06ff, 0,
					Node.TYPE_BUFF1_INDEX),

			new Node("all_settings_key", 0xc6c701, 0, 0),
			new Node("driver_assist_key", 0xc6c101, 0, 0),
			new Node("parking_and_manoeuvring_key", 0xc6c201, 0, 0),
			new Node("lights_key", 0xc6c301, 0, 0),
			new Node("mirrors_and_wipers_key", 0xc6c401, 0, 0),
			new Node("opening_and_closing_key", 0xc6c501, 0, 0),
			new Node("multifunction_display_key", 0xc6c601, 0, 0),
			new Node("ambient_lighting_reset", 0xc6c901, 0, 0),
			new Node("electric_driving_charging_hybrid", 0xc6ca01, 0, 0),
			new Node("seat_settings", 0xc6ce01, 0, 0),

			new Node("reset_long_term_key", 0xc68a, 0, 0),
			new Node("reset_since_start_key", 0xc68b, 0, 0),

			new Node("dynamic_light_assistant", 0xc65a, 0x40510000, 0x2000000),

			new Node("motorway_light", 0xc659, 0x40510000, 0x1000000),

			new Node("headlight_distance_adjustment_key", 0xc65f, 0x40510000,
					0xe),
			...
			new Node("brightness_of_variable_channel_auxiliary_system", 0xc63f,
					0x40310000, 0xf00000),
			new Node("preventive_member_protection_system_key", 0xc63e,
					0x40300000, 0x200),
			new Node("lane_assist_activited_key", 0xc636, 0x40300000, 0x100),

			new Node("direct_tire_pressure_test_key", 0xc623, 0x40200000,
					0xff0000),
			new Node("speed_warning_at_key", 0xc621, 0x40200000, 0xff00),
			new Node("driver_mode", 0xc6d0, 0x40a00000, 0xf),
			...
			new Node("slow_down", 0xc639, 0x40310000, 0x10000, 0x0),
			new Node("blind_area_monitoring", 0xc63d, 0x40310000, 0x20000, 0x0),
			new Node("alert_prompt", 0xc63C, 0x40310000, 0xC0000, 0x0),
			...
			new Node("wiper_maintenance_position_key", 0xc68c, 0x63220000, 0x1),
			new Node("parking", 0xc664, 0x40600000, 0x100),

			new Node("acousticconfirmation", 0xc675, 0x40700000, 0x4000),
			...
			new Node("remote_control_key_memory_matching", 0xc6CB, 0x40B00000,
					0x2),
			new Node("driver_seat_key", 0xc6C9, 0x40B00000, 0x4),
			new Node("front_window_key", 0xc666, 0x40700000, 0x400),
			new Node("rear_window_key", 0xc667, 0x40700000, 0x800),
			new Node("sliding_window_key", 0xc668, 0x40700000, 0x1000),
			new Node("induction_rear_rear_cover", 0xc674, 0x40700000, 0x200),
			...
			new Node("vw_meter_mode", 0xc6e9, 0x40c10000, 0xff),
			new Node("vw_left_meter", 0xc6ea, 0x40c10000, 0xff00),
			new Node("vw_right_meter", 0xc6eb, 0x40c10000, 0xff0000),
			...
			new Node("individual", 0xc6a0, 0x40b20000, 0x80),
			new Node("user_account_key", 0xc6a1, 0x40b20000, 0x70),
			new Node("key_distribution_key", 0xc6a2, 0x40b20000, 0x8),
			new Node("assign_the_key_the_current_user_account_key", 0xc6a3,
					0x0, 0x0),
			...
			new Node("reset_main_info", 0xc6d4, 0x0, 0x0), };

//初始状态
	private final static int[] INIT_CMDS = { 0x6310,0x6311,0x6320,0x6321,0x6322,0x6300,
		0x4000, 0x4010, 0x4020, 0x4030, 0x4031, 0x4040, 
		0x4050, 0x4051, 0x4052, 0x4053, 0x4060, 0x4070, 
		0x4080, 0x4090, 0x40a0, 0x40b0, 0x40b1, 0x40b2, 0x40c0, 0x40c1,
		0x51ff, 0x5080,
	/*
...
//			((PreferenceScreen)findPreference("service_settings")).removePreference(findPreference("oil_data"));
	...
	public boolean onPreferenceClick(Preference arg0) {
		if (arg0.getKey().equals("individual_reset")) {
			...
			.setTitle(R.string.reset_main_info)
	...
	private void updateVisible(byte[] buf) {
		...
		case 0x31:
			...
			showPreference("showdistancecontrol", buf[3] & 0x8,
					"driverassistance");
			showPreference("accrdriveprogram", buf[3] & 0x10,
					"driverassistance");
	...
	private void updateView(byte[] buf) {
		...
		if ((buf[0] & 0xff) == 0x40 && (buf[2] & 0xff) == 0xb2) {// driver
			...
			if ((buf[3] & 0x80) != 0) {
				...
				showPreferenceEnable(
						"assign_the_key_the_current_user_account_key", true);
				...
				showPreferenceEnable("key_distribution_key", false);
				showPreferenceEnable(
						"assign_the_key_the_current_user_account_key", false);
			}

 ...
 */
public class CanboxSettings2 extends PreferenceActivity implements Preference.OnPreferenceChangeListener, OnPreferenceClickListener, OnClickListener {
    ...
    private void updateConfig(String name) {
        ...
        mConfigurationPreference.setSummary(mConfigurationPreference.getEntry());
    }
    ...
    private void initCanMachineConfig() {
        ...
        if (mProId != null && mProId.length() >= 4) {
            ...
            MMLog.d(TAG, mManaId + ":" + mCateId + ":" + mModelId);

            mManufacturerName = mXlmParser.getMenaByValue(mManaId);
            if (mCategoryName == null) {
                mCategoryName = mXlmParser.getCategorysByValue(mCateId);
                if (mCategoryName != null) {
                    mModelName = mXlmParser.getModelByValue(mCategoryName, mModelId);
                }
            }
            MMLog.d(TAG, mManufacturerName + ":" + mCategoryName + ":" + mModelName);
        }
    ...
    private String getExt2Config() {
        ...
        if (mAirCondition != null) {
            s += "," + MachineConfig.KEY_SUB_CANBOX_AIR_CONDITION + mAirCondition;
        }
        if (mChangeKey != null) {
    ...
    private void setCanSettings() {
        ...
        if ((mExternalRadar != null) && (!mExternalRadar.equals(mExternalRadarOrg))) {
            ...
            MachineConfig.setProperty(MachineConfig.KEY_EXTERNAL_BOX, mExternalRadar);
        }
                ...
                if (canSetting.mPro != null) {
                    try {
                        ...
                        if (mCarOtherSettings != null) {
                            s += "," + MachineConfig.KEY_SUB_CANBOX_OTHER + mCarOtherSettings;
                        }
                        ...
                        if (mAppShow != null) {
                            if (mAirCondition != null) {
                                ...
                                try {
                                    acFlag = Integer.parseInt(mAirCondition);
                                } catch (Exception ignored) {
                                }
                                    ...
                                    if (mAppShow.contains(AppConfig.HIDE_CANBOX_AC)) {
                                        ...
                                        if (ss.length > 1) {
                                            ...
                                            for (String string : ss) {
                                                if (!AppConfig.HIDE_CANBOX_AC.equals(string)) {
                                                    mAppShow += string + ",";
                                                }
                                            }
                                        }
        ...
        if (mManufacturerName != null && mManufacturerName.equals(mXlmParser.getTranslation("通用机"))) {
            MachineConfig.setProperty(MachineConfig.KEY_CAN_BOX_PG_SWITCH, mCarSettingsValue);
            Util.sudoExec("sync");
            Toast.makeText(this, R.string.success, Toast.LENGTH_LONG).show();
        } else {
        ...
        MachineConfig.setProperty(MachineConfig.KEY_CAN_BOX_SHOW_APP, null);
    ...
    private void canboxUpdate(String manufacturerId) {
        try {
            String s = null;
            ...
            it.setPackage(AppConfig.getCarServicePackageName(this));
            sendBroadcast(it);
        } catch (Exception ignored) {
        }
    }
    ...
    private void updateAdvancedVisible(boolean show) {
        ...
        if (show) {
            if (findPreference("canbox_front_door") == null && mLPFrontDoor != null) {
                getPreferenceScreen().addPreference(mLPFrontDoor);
            }
            if (findPreference("canbox_rear_door") == null && mLPBackDoor != null) {
            ...
            if (findPreference("canbox_key_change_ex") == null && mKeyChangeSettings != null) {
            ...
            //			}
            if (findPreference("canbox_other_settings") == null && mCarSettings != null) {
    ...
    private void updateOtherSettings() {
        ...
        String[] entry2 = {
                ...
                getResources().getString(R.string.time_hour_add_1), getResources().getString(R.string.time_hour_minus_1), getResources().getString(R.string.data_distribution)};
        ...
        if (mCarOtherSettings != null) {
            try {
                ...
                for (int i = 0; i < 32; i++) {
                    if ((v & (0x1 << i)) != 0) {
                        ss.add("" + i);
                        summary.append(entry2[i]).append(" ");
                    }
                }
            } catch (Exception ignored) {
            }
    ...
    private void updateACSettings() {
        ...
        try {
            ...
            if (mModel != null) {
                ...
                if (mAppShow != null && mAppShow.contains(AppConfig.HIDE_CANBOX_AC)) {
                    forceShowAC = false;
                }
            }
        } catch (Exception e) {

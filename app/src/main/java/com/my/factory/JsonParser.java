...
public class JsonParser {
	...
	public String buildAppShow(String manaId, String cateId, String modelId,
			String pro, String config) {
		String show = "";
		for (int i = 0; i < mCanAppShows.size(); ++i) {
			CanAppShow cs = mCanAppShows.get(i);
			boolean set = false;
			if (cs.mPros != null && pro != null) {
				for (String s : cs.mPros) {
					...
					if (ss[0].equals(pro)) {
						if (ss.length == 1
								|| (ss.length > 1 && ss[1].equals(config))) {
							show += cs.mApp + ",";
							set = true;
							break;
						}
			...
			if (!set) {
				if (cs.mIds != null) {
					for (CanAppShowIDS ids : cs.mIds) {
						if (ids.mMenaID.equals(manaId)
								&& ids.mCategoryID.equals(cateId)) {
	...
	private void parserAppShow(Context c) {
		...
		try {
			...
			InputStreamReader inputStreamReader = new InputStreamReader(
					assetManager.open("can_apps_show.json"), "UTF-8");
	...
	private void parserCanBaud(Context c) {
		...
		try {
			AssetManager assetManager = c.getAssets();
			InputStreamReader inputStreamReader = new InputStreamReader(
					assetManager.open("can_baud_config.json"), "UTF-8");
			BufferedReader br = new BufferedReader(inputStreamReader);
			String line;
			StringBuilder builder = new StringBuilder();
			while ((line = br.readLine()) != null) {
